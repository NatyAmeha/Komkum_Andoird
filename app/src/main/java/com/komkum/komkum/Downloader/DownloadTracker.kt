...
class DownloadTracker @Inject constructor(
    ...
    fun getCurrentDownloadItem(id: String) = getAllCurrentlyDownloadedItems().find { download -> download.request.id == id }
...
//        allDownloadList.value = downloadList
    ...
    fun addProgressiveDownload(id : String , uri : Uri){
        downloadHelper = DownloadHelper.forProgressive(context , uri)
        downloadHelper!!.prepare(DownloadHelperCallback(id , uri.toString()))
    }
...
//                        if(!it) Toast.makeText(context, "you download this song more than 3 times", Toast.LENGTH_LONG).show()
//                    }
                }
            }
        }
        ...
        var newUri = when(qualitySelector){
            "64" -> Uri.parse(uri.toString().replace("index.m3u8" , "hls_audio_64k_v4.m3u8"))
            "160" -> Uri.parse(uri.toString().replace("index.m3u8" , "hls_audio_160k_v4.m3u8"))
            else -> Uri.parse(uri.toString().replace("index.m3u8" , "hls_audio_64k_v4.m3u8"))
        }

        downloadHelper = DownloadHelper.forHls(context , newUri , dataSourceFactory , renderersFactory )
        downloadHelper!!.prepare(DownloadHelperCallback(id , uri.toString()))
    }
    ...
    fun addDownloads(streamableList: List<Streamable<String, String>> , lifecycleOwner: LifecycleOwner , downloadtype: Int? = null){
      getCompletedDownloads().observe(lifecycleOwner, Observer {
          ...
          streamableList.forEach { song ->
              var uri = Uri.parse(song.mpdPath)
              addDownload(song._id  , uri , downloadtype)
          }
      })
    }
...
//                    progressCompleteCallback()
    ...
    inner class DownloadHelperCallback(var id: String , var url : String) : DownloadHelper.Callback{
        override fun onPrepared(helper: DownloadHelper) {
            var trackKeys = mutableListOf<TrackKey>()
            Log.i("trackperiod", helper?.periodCount.toString())
...
//                        if(track.bitrate > 0) trackKeys.add(TrackKey(trackGroups, trackGroup, track))
//                    }
//                }
//            }
...
//            }
            ...
            var preference = PreferenceHelper.getInstance(context)
            var qualitySelector = preference.get("download_quality", "64")
            Log.i("downloadselectedquality", qualitySelector)
            startAdaptiveDownload( helper , id )
...
//            helper?.release()
    ...
    fun startAdaptiveDownload(helper: DownloadHelper? , id : String ){
...
//            Log.i("trackstreamkey" , mappedTrackInfo.rendererCount.toString() + " "+ periodIndex.toString())
//            for (i in 0 until mappedTrackInfo.rendererCount) {
////                var rendererTrackGroups = mappedTrackInfo.getTrackGroups(i);
...
//            Log.i("trackstreamkey" , "none selected")
//            return
//        }

//        var hlsDownlader = HlsDownloader(Uri.parse(url) , downloadRequest.streamKeys,
//        DownloaderConstructorHelper(cache,dataSourceFactory)
//        )
//        hlsDownlader.download { contentLength, bytesDownloaded, percentDownloaded ->  }

//        DownloadService.sendAddDownload(context, MediaDownloaderService::class.java, downloadRequest, false)
        if (downloadRequest != null) {
            downloadManager.addDownload(downloadRequest)
            DownloadService.sendSetStopReason(context, MediaDownloaderService::class.java, downloadRequest?.id, Download.STOP_REASON_NONE, false)
        }
        helper?.release()
    }
